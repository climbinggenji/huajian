<template>
  <div>
    ...
    <!-- <p class="next contain"><span>Next Project</span></p> -->
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    img1() {
      return require('@/assets/imgs/retail/'+this.$route.query.file+'/1.jpg')
    },
    text() {
    ...
    imgList() {
      ...
      for (let index = 1; index < parseInt(this.$route.query.count); index++) {
        arr.push(require('@/assets/imgs/retail/'+this.$route.query.file+'/'+(index+1)+'.jpg'))
      }
      return arr
    }
  },
